...
public class AddCombinationsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        shoes.setOnClickListener(new View.OnClickListener() {
            ...
            @Override
            public void onClick(View v) {
                if (isEmptySelectedImages()==true){
                    Toast.makeText(v.getContext(),"Combination slots should not be empty",Toast.LENGTH_LONG).show();
                }else {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode==1){
            if (resultCode==RESULT_OK){
                ...
                selectedimages[0]=sqLiteHelper.getClothing(result).getImg();
        ...
        if(requestCode==2){
            if (resultCode==RESULT_OK){
                int result=data.getIntExtra("result",0);
                selectedimages[1]=sqLiteHelper.getClothing(result).getImg();
        ...
        if(requestCode==3){
            if (resultCode==RESULT_OK){
                int result=data.getIntExtra("result",0);
                selectedimages[2]=sqLiteHelper.getClothing(result).getImg();
        ...
        if(requestCode==4){
            if (resultCode==RESULT_OK){
                int result=data.getIntExtra("result",0);
                selectedimages[3]=sqLiteHelper.getClothing(result).getImg();
        ...
        if(requestCode==5){
            if (resultCode==RESULT_OK){
                int result=data.getIntExtra("result",0);
                selectedimages[4]=sqLiteHelper.getClothing(result).getImg();
    ...
    public void fillComponents(int id){
        ...
        selectedimages[0]=sqLiteHelper.getClothing(combination.getOverhead().getId()).getImg();
        selectedimages[1]=sqLiteHelper.getClothing(combination.getFace().getId()).getImg();
        selectedimages[2]=sqLiteHelper.getClothing(combination.getTorso().getId()).getImg();
        selectedimages[3]=sqLiteHelper.getClothing(combination.getLegs().getId()).getImg();
        selectedimages[4]=sqLiteHelper.getClothing(combination.getFeet().getId()).getImg();
    ...
    public boolean isEmptySelectedImages(){
